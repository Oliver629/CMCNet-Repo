...
from straug.process import Posterize, Invert, Equalize
...
class UnetDataset(Dataset):
    def __init__(self, annotation_lines, input_shape, num_classes, train, dataset_path):
        ...
        self.train = train
        self.dataset_path = '/20TB/data/yixue/backups/'
    ...
    def __getitem__(self, index):
        ...
        #   从文件中读取图像
        # -------------------------------#
        jpgpath = cv2.imread(os.path.join('/20TB/data/yixue/allimg/', name + ".jpg"), 0)
        jpg = cv2.resize(jpgpath, self.input_shape)       # [448, 448]
        if jpg is None:
            print(f"Failed to load image from {jpgpath}")
        ...
        for i in range(self.num_classes):
            label = cv2.imread(os.path.join("/20TB/data/yixue/true_labels/", str(i), name + '.jpg'), 0)
